...
import { FlaskConical, Barcode, CheckCircle, Clock, Ban, Search, Send, ArrowRight, FileText, AlertTriangle, Activity, Eye, Beaker } from 'lucide-react';
...
const Laboratoire = () => {
  ...
  // Build all lab requests from consultations
  const allLabRequests = MOCK_PATIENTS.flatMap(p =>
  ...
  const extraRequests = [
    ...
    { id: 'extra-3', patient: 'Djimé Koumba', patientId: '14', nhid: 'TCD-2024-00014', examen: 'ECBU + Créatinine', docteur: 'Dr. Hassan Idriss', service: 'Urologie', date: '2024-03-08', paye: true, statut: 'en_cours' as const, urgence: 3 as const },
  ];
  ...
  const urgenceColor = (u: number) => u <= 2 ? 'text-destructive' : 'text-foreground';

  const handleScanBarcode = () => {
    setScanResult('TCD-2024-00001-L001');
    toast.info('🔍 Code-barres scanné: TCD-2024-00001-L001', { description: 'Échantillon identifié – Mahamat Abdoulaye' });
  };
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm">Gestion complète des analyses, suivi des échantillons et résultats</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {[
          ...
          { label: 'En cours', value: String(allRequests.filter(r => r.statut === 'en_cours').length), icon: FlaskConical, color: 'text-primary' },
      ...
      <Tabs defaultValue="demandes" className="space-y-4">
        <TabsList className="bg-muted/60">
          <TabsTrigger value="demandes">📋 Demandes d'analyses</TabsTrigger>
                    ...
                    <div className="flex items-center gap-3 flex-1">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          <p className={`font-semibold text-sm ${urgenceColor(req.urgence)}`}>{req.patient}</p>
                    ...
                    <div className="flex items-center gap-2">
                      {req.paye ? (
                        ...
                        <Badge variant="destructive" className="text-[10px] gap-1">
                          <Ban className="w-3 h-3" /> Non payé
                        </Badge>
                      )}
                      ...
                      {req.statut === 'en_attente' && (
                        <Button size="sm" className="h-7 text-xs" disabled={!req.paye}
                          onClick={() => toast.success(`Analyse "${req.examen}" lancée pour ${req.patient}`, { description: 'Échantillon à prélever' })}>
                          {req.paye ? 'Lancer' : 'Payer d\'abord'}
                        </Button>
                      )}
                      {req.statut === 'en_cours' && (
                        <Button size="sm" variant="outline" className="h-7 text-xs gap-1"
                          onClick={() => toast.success(`Résultats validés et envoyés au DPI`)}>
        ...
        <TabsContent value="suivi" className="space-y-4">
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              <div className="flex gap-2">
                <Input placeholder="Scannez ou saisissez le code-barres" value={scanResult} onChange={e => setScanResult(e.target.value)} />
                <Button onClick={handleScanBarcode} variant="outline" className="gap-1"><Barcode className="w-4 h-4" /> Scanner</Button>
              </div>
              {scanResult && (
                <div className="p-3 rounded-lg border border-primary/30 bg-primary/5">
                  <p className="text-sm font-medium text-foreground">Échantillon identifié: {scanResult}</p>
                  <p className="text-xs text-muted-foreground">Patient: Mahamat Abdoulaye • Examen: Goutte épaisse + NFS</p>
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Traçabilité complète de la prescription au résultat dans le DPI</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {allRequests.filter(r => r.paye).slice(0, 6).map((req, idx) => {
        ...
        <TabsContent value="resultats" className="space-y-4">
          {completedResults.length === 0 ? (
            <Card><CardContent className="p-8 text-center text-muted-foreground">Aucun résultat disponible</CardContent></Card>
            ...
            <Card key={result.id} className="hover:shadow-sm transition-shadow">
              <CardContent className="p-4 space-y-3">
                ...
                <div className="rounded-lg border border-border overflow-hidden">
                  ...
                  {result.resultats.map((r, i) => (
                    <div key={i} className={`grid grid-cols-4 gap-0 text-xs p-2 border-t border-border ${r.statut === 'anormal' ? 'bg-destructive/5' : ''}`}>
                      ...
                      <span>
                        {r.statut === 'anormal' ? (
                          <Badge variant="destructive" className="text-[9px]">⚠ Anormal</Badge>
                        ) : (
                          <Badge variant="outline" className="text-[9px] text-secondary border-secondary">Normal</Badge>
                        )}
                      </span>
                    </div>
                  ))}
                ...
                <div className="flex gap-2">
                  <Button size="sm" variant="outline" className="text-xs gap-1" onClick={() => toast.info('Résultats ouverts dans le DPI')}>
                    <Eye className="w-3 h-3" /> Voir dans DPI
                  </Button>
                  <Button size="sm" variant="outline" className="text-xs gap-1" onClick={() => toast.success('Résultats imprimés')}>
                    <FileText className="w-3 h-3" /> Imprimer
                  </Button>
                </div>
              </CardContent>
            </Card>
        ...
        <TabsContent value="automates" className="space-y-4">
              ...
              <Card key={auto.name}>
                <CardContent className="p-4 space-y-3">
                  ...
                  <div className="flex justify-between items-center p-2 rounded bg-muted/40">
                    <span className="text-xs text-muted-foreground">Tests réalisés aujourd'hui</span>
                    <span className="text-sm font-bold text-foreground">{auto.tests}</span>
                  </div>
                  <Button className="w-full text-xs gap-1" variant="outline"
                    onClick={() => toast.success(`Résultats de ${auto.name} reçus et intégrés au DPI`)}>
                    <FlaskConical className="w-3 h-3" /> Recevoir résultats
                  </Button>
                </CardContent>
              </Card>
            ))}
          ...
          <Card className="border-secondary/30 bg-secondary/5">
            <CardContent className="p-4">
              <p className="text-sm font-bold text-foreground">🔗 Interface Automates – Statut des connexions</p>
              <p className="text-sm text-foreground mt-1">Tous les automates sont connectés. Les résultats sont automatiquement poussés vers le DPI du patient via le protocole HL7/ASTM. Dernière synchronisation: il y a 2 minutes.</p>
            </CardContent>
          </Card>
        </TabsContent>
      </Tabs>
